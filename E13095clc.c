...
int main(int argc,char**argv){
	...
	if(argc!=4){//error if the number of arguments are wrong
     ...
     if(positionerror(argv[3],argv[1])!=-1){fprintf(stderr, "ERROR: Input format error at location %d.",positionerror(argv[3],argv[1]));}//checking the position of wrong input
     else if( stringlength(argv[3])==64){fprintf(stderr, "ERROR: This conversion is not possible." );}//int is 32 bit
      ...
      if(positionerror(argv[3],argv[1])!=-1){fprintf(stderr, "ERROR: Input format error at location %d.",positionerror(argv[3],argv[1]));}//checking the position of wrong input
      else{fprintf(stdout, "0x%s",argv[3]);}
    }    
     ...
     if(positionerror(argv[3],argv[1])!=-1){fprintf(stderr, "ERROR: Input format error at location %d.",positionerror(argv[3],argv[1]));}//checking the position of wrong input
      ...
      if(positionerror(argv[3],argv[1])!=-1){fprintf(stderr, "ERROR: Input format error at location %d.",positionerror(argv[3],argv[1]));}//checking the position of wrong input
      else if(stringlength(argv[3])==16 ){fprintf(stderr, "ERROR: This conversion is not possible." );}//int is represented by 32 bit
     ...
     if(positionerror(argv[3],argv[1])!=-1){fprintf(stderr, "ERROR: Input format error at location %d.",positionerror(argv[3],argv[1]));}//checking the position of wrong input
     else if(errorint(argv[3])==-1){fprintf(stderr, "ERROR: This conversion is not possible." );}  //to make sure it doesnt overfloaw
    ...
    if(positionerror(argv[3],argv[1])!=-1){fprintf(stderr, "ERROR: Input format error at location %d.",positionerror(argv[3],argv[1]));}//checking the position of wrong input
    else if(errorint(argv[3])==-1){fprintf(stderr, "ERROR: This conversion is not possible." );}//if int overflows
...
int errorint(char a[]){//to check if the integer is more than the max 32 bit value
 long long val=(long long)atof(a);
...
int errorfloat(char a[]){////to check if the float is more than the max 32 bit value
...
if(val>max || val<min ){
  return -1;}
   ...
   //didnt check max value of double because they are impractical     
...
int stringcompare(char a[],char b[]){//checking if two string are equal
...
long long hexatoint(char a[]){//funtion to convert hexa decimals to integers
  long long n,sum=0;
  float temp;
...
i=i++;
   ...
   long long modinteger=-integer;//ignoring the negative value for now
   int  k,n=0,j,carry=1;
   int onesComp[32],twoscomp[32];
    while(modinteger!=0) {//getting the binary part
        binarynum[n]=modinteger%2;
...
int k=(int)(siginificand*1000000000);
for(n=9;n<33;n++){//filling up the significand 
  	
  	int z=(int)(pow(2,-i)*1000000000);
