...
int main(int argc,char**argv){
	...
	if(argc!=4){//error if the number of arguments are wrong
     ...
     fprintf(stdout, "%s",bintobin(argv[3]) );
    }
     ...
     if(bintoint(argv[3])==-1 || stringlength(argv[3])==64){fprintf(stderr, "ERROR: This conversion is not possible." );}//to make sure it doesnt overfloaw
     else{ fprintf(stdout,"%d",bintoint(argv[3]) );}
    }
    else if(stringcompare(argv[1],b)==0 && stringcompare(argv[2],d)==0){//binary to float
            if(stringlength(argv[3])==64){fprintf(stderr, "ERROR: This conversion is not possible." );}//as float is represented by 32 bit only
          else{  fprintf(stdout, "%0.2f",binarytofloat(argv[3]) );}

    }
    else if(stringcompare(argv[1],b)==0 && stringcompare(argv[2],e)==0){//binary to double
          if(stringlength(argv[3])==32){fprintf(stderr, "ERROR: This conversion is not possible." );}//double is represented by 64 bit
              else{fprintf(stdout, "%0.6f",binarytodouble(argv[3]) );}
    }
    else if(stringcompare(argv[1],a)==0 && stringcompare(argv[2],a)==0){//hexa to hexa
      fprintf(stdout, "%s",hexatohexa(argv[3]) );
    }    
     ...
     hexatobin(argv[3]);
       ...
       if(stringlength(argv[3])==16 || hexatoint(argv[3])==-1){fprintf(stderr, "ERROR: This conversion is not possible." );}//int is represented by 32 bit
       else {fprintf(stdout, "%d",hexatoint(argv[3]) );}
    }
    else if(stringcompare(argv[1],a)==0 && stringcompare(argv[2],d)==0){//hexa to float
        if(stringlength(argv[3])==16){fprintf(stderr, "ERROR: This conversion is not possible." );}//float represented by 32 bits
      else{ fprintf(stdout, "%0.2f",hexatofloat(argv[3]) );}
    }
    else if(stringcompare(argv[1],a)==0 && stringcompare(argv[2],e)==0){//hexa to double
        if(stringlength(argv[3])==8){fprintf(stderr, "ERROR: This conversion is not possible." );}//double represented by 64 bits
