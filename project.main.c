...
int main(int argc,char**argv){
    ...
    fprintf(stderr, "ERROR: This conversion is not possible." );//can convert 32 bit to 64 bit
    }
    else if(stringcompare(argv[1],d)==0 && stringcompare(argv[2],d)==0 ){//float to float
       if(errorfloat(argv[3])==-1){fprintf(stderr, "ERROR: This conversion is not possible." );} 
...
else if(stringcompare(x,"-I")==0){//if input isint
int count=0;
 for(n=0;n<size;n++){//to check if there is a - value inside the number
